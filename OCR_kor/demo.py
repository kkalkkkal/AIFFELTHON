...
def post() : #post 요청시 
    ...
    #*
    #* */
    os.remove("./workspace/demo_images") # 기존 파일 삭제

    # json에 들어온 파일 저장
    for v in images:
        image_data = images["image"]
        binary_np = np.array(image_data["data"], dtype=np.uint8)
        # data cv2 np convert
        ...
        cv2.imwrite('./workspace/demo_images'+image_data["result_title"]+image_data["timestamp"]+'.'+image_data["format"], imen)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_folder', default="./workspace/demo_images", help='path to image_folder which contains text images. required=True')
    ...
    parser.add_argument('--Prediction', type=str, default='CTC', help='Prediction stage. CTC|Attn. required=True')
